...
Models are not the only way to write API logic in Cloesce. Services are another core concept in Cloesce that allow you to encapsulate business logic and share it across your application. Services are similar to Models in that they can define methods that can be called from your API routes, but they do not have any associated data storage or schema. Instead, Services are used to group related functionality together and can be injected into other parts of your application using Cloesce's dependency injection system.
...
To share dependencies across your Cloesce application methods, Cloesce enables dependency injection. By default, Cloesce provides two core dependencies:
...
Services can also be injected into other Services. They cannot be circular dependencies, but otherwise, you can freely compose Services together.
