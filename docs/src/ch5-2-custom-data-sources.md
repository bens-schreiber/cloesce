...
It is likely that the [Default Data Source](./ch5-1-overview.md#default-data-source) generated by Cloesce will not fit all of your needs for data retrieval. For example, you may want to fetch a `Person` without their associated `Dog`, or fetch a `Profile` without its `avatar` [R2 field](./ch4-5-r2-fields.md).

Cloesce allows you to define custom Data Sources for any Model, which can include any combination of [KV](./ch4-4-kv-fields.md), [R2](./ch4-5-r2-fields.md), and [Navigation Fields](./ch4-3-d1-navigation-fields.md), in addition to custom SQL queries for [D1 backed Models](./ch4-1-d1-backed-model.md).
...
The above code will be generated by default for any D1 backed Model, but can be overridden by defining a Data Source called `Default`, or by making a custom data source such as:
