...
/**
 * Generated surface for the `PooAcceptYield` model.
 *
 * Implement via {@link impl}. Access data through data sources, not this namespace directly.
 *
 * @example
 * ```ts
 * export const PooAcceptYield = clo.PooAcceptYield.impl({
 *   async someApi(self, env, arg) {
 *     const row = await PooAcceptYield.Default.get(env, arg);
...
export namespace PooAcceptYield {
    ...
    export namespace GeneratedSource {
        ...
        export const Default = {
            ...
            selectQuery: `SELECT "PooAcceptYield"."id" AS "id" FROM "PooAcceptYield"`,

            /** Build the bound `D1PreparedStatement` for this data source's `get`. */
            getQuery(env: { db: CfEnv["db"] }, id: number): D1PreparedStatement {
                return env.db.prepare(`SELECT "PooAcceptYield"."id" AS "id" FROM "PooAcceptYield" WHERE "PooAcceptYield"."id" = ?1`).bind(id);
            },
            ...
            listQuery(env: { db: CfEnv["db"] }, lastSeen_id: number, limit: number): D1PreparedStatement {
                return env.db.prepare(`SELECT "PooAcceptYield"."id" AS "id" FROM "PooAcceptYield" WHERE "PooAcceptYield"."id" > ?1 ORDER BY "PooAcceptYield"."id" ASC LIMIT ?2`).bind(lastSeen_id, limit);
            },
            /**
             * Load a single `PooAcceptYield`.
             *
            ...
            /**
             * Load all matching `PooAcceptYield` rows.
             *
            ...
            /**
             * Insert or update a `PooAcceptYield` and its included relations.
             *
    ...
    /**
     * Register your implementation of `PooAcceptYield`.
     *
     ...
     *     const res = await PooAcceptYield.Default.get(env, arg);
     *     return res.data!;
     *   },
     * });
     * ```
    ...
    export function impl<Impl extends Api & Sources>(implObj: Impl & ThisType<{ tag: string; Orm: typeof Orm; Default: typeof GeneratedSource.Default } & Impl>): { tag: string; Orm: typeof Orm; Default: typeof GeneratedSource.Default } & Impl {
        return _impl("PooAcceptYield", { Orm, Default: GeneratedSource.Default }, implObj) as any;
    }

    /**
     * Lower-level ORM access with explicit {@link IncludeTree}. Returns raw `CloesceResult`.
     *
     * Prefer data sources (e.g. `PooAcceptYield.Default.save(...)`) unless you need a custom tree or query.
     */
    export namespace Orm {
        /**
         * Insert or update `PooAcceptYield` and its included relations.
         *
         ...
         */
        export async function list(env: { db: CfEnv["db"] }, args: { query?: D1PreparedStatement, include?: IncludeTree<Self> }): Promise<CloesceResult<PooAcceptYield.Self[]>> {
            args.include ??= GeneratedSource.Default.tree;
            return await CloesceOrm.fromEnv(env).list<Self>(Meta, args.query, args.include);
        }

        /**
         * Hydrate an already-fetched `D1Result` into `PooAcceptYield` rows.
         *
        ...
        /**
         * Hydrate a partial `PooAcceptYield` into a full row, loading the included relations.
         *
...
export function cloesce(env: CfEnv): CloesceApp {
    // @ts-expect-error
    return new CloesceApp(cidl as any, "http://localhost:5005/api", upgradeEnv(env));
}
