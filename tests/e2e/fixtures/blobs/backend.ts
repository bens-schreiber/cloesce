...
/**
 * Generated surface for the `BlobHaver` model.
 *
 * Implement via {@link impl}. Access data through data sources, not this namespace directly.
 *
 * @example
 * ```ts
 * export const BlobHaver = clo.BlobHaver.impl({
 *   async someApi(self, env, arg) {
 *     const row = await BlobHaver.Default.get(env, arg);
...
export namespace BlobHaver {
    ...
    export namespace GeneratedSource {
        ...
        export const Default = {
            ...
            /**
             * Load all matching `BlobHaver` rows.
             *
            ...
            /**
             * Insert or update a `BlobHaver` and its included relations.
             *
    ...
    /**
     * Register your implementation of `BlobHaver`.
     *
     ...
     *     const res = await BlobHaver.Default.get(env, arg);
     *     return res.data!;
     *   },
     * });
     * ```
     ...
     *
     * Prefer data sources (e.g. `BlobHaver.Default.save(...)`) unless you need a custom tree or query.
     */
    export namespace Orm {
        /**
         * Insert or update `BlobHaver` and its included relations.
         *
         ...
         */
        export async function list(env: { db: CfEnv["db"] }, args: { query?: D1PreparedStatement, include?: IncludeTree<Self> }): Promise<CloesceResult<BlobHaver.Self[]>> {
            args.include ??= GeneratedSource.Default.tree;
            return await CloesceOrm.fromEnv(env).list<Self>(Meta, args.query, args.include);
        }

        /**
         * Hydrate an already-fetched `D1Result` into `BlobHaver` rows.
         *
        ...
        /**
         * Hydrate a partial `BlobHaver` into a full row, loading the included relations.
         *
...
/**
 * Generated surface for `BlobService` (no persisted data, pure API).
 *
 ...
 * @example
 * ```ts
 * export const BlobService = clo.BlobService.impl({
...
export namespace BlobService {
    ...
    /** Methods to implement for `BlobService`. Each returns an {@link ApiResult}. */
    export interface Api {
        incrementBlob(
            b: Uint8Array,
        ): ApiResult<Uint8Array>;
...
export function cloesce(env: CfEnv): CloesceApp {
    // @ts-expect-error
    return new CloesceApp(cidl as any, "http://localhost:5270/api", upgradeEnv(env));
}
