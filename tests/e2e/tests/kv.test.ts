...
describe("StreamKV", () => {
  ...
  it("Performs PUT method", async () => {
    const res = await StreamKV.put(key);
    expect(res.ok, withRes("PUT failed", res)).toBe(true);
  });

  let kv: StreamKV;
  it("Retreives StreamKV", async () => {
    const res = await StreamKV.get(key);
    expect(res.ok, withRes("GET failed", res)).toBe(true);
    kv = res.data;

    expect(kv instanceof StreamKV).toBe(true);
    expect(kv.key).toBe(key);
    expect(kv.metadata).toBeDefined();
  });

  it("Uploads a stream to KV", async () => {
    ...
    const res = await kv.putStream(stream);
    expect(res.ok, withRes("PUT Stream failed", res)).toBe(true);
  });

  it("Retrieves the stream from KV", async () => {
    const res = await kv.getStream();
    expect(res.ok, withRes("GET Stream failed", res)).toBe(true);

    const got: number[] = Array.from(res.data);
    expect(value.length === got.length && value.every((v, i) => v === got[i]));
  });
});
