...
import { PureKVModel, D1BackedModel } from "../../fixtures/regression/kv/client.js";
...
describe("PureKVModel", () => {
    ...
    it("POST", async () => {
        const res = await PureKVModel.post(id, data, otherData);
...
describe("D1BackedModel", () => {
    ...
    it("POST", async () => {
        ...
        const res = await D1BackedModel.post(model);
        expect(res.ok, withRes("POST should be OK", res)).toBe(true);
    });
