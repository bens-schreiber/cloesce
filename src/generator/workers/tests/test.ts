...
$.ExampleModel = {
  validate(obj: any): boolean {
    ...
    if (obj.rating == null || typeof obj.rating !== "number") {
      return false;
    }
    ...
    if (
      obj.data_blob == null ||
      !(
        obj.data_blob instanceof ArrayBuffer ||
        obj.data_blob instanceof Uint8Array
      )
    ) {
      return false;
    }
    ...
    ExampleModel: {
      ...
      process_models: async (request: Request, env: Env) => {
        ...
        if (
          ...
          models.some((item) => item == null || !$.ExampleModel.validate(item))
        ) {
          return new Response(
            JSON.stringify({ error: "Invalid request parameters" }),
            {
