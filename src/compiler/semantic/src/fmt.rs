...
fn display(
    ...
    match error {
        ...
        SemanticError::D1ModelMissingD1Binding { model } => {
            ...
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
                .with_message(format!(
                    "model '{}' has columns but no `@d1` binding is specified",
                    model.name
                ))
                .with_label(
                    Label::new((path, range))
                        .with_message("add a `@d1(\"<binding>\")` tag to this model")
                        .with_color(Color::Red),
                )
                .finish()
                .write(cache, std::io::stderr())
        ...
        SemanticError::D1ModelInvalidD1Binding { model, tag } => {
            let (model_path, model_range) = span_parts(&model.span, file_table);
            let (tag_path, tag_range) = span_parts(&tag.span, file_table);
            Report::build(
                ariadne::ReportKind::Error,
                (tag_path.clone(), tag_range.clone()),
            )
            .with_message(format!(
            ...
            .with_label(
                Label::new((tag_path, tag_range))
                    .with_message("this binding does not exist")
                    .with_color(Color::Red),
            )
            .with_label(
        ...
        SemanticError::D1ModelMissingPrimaryKey { model } => {
            ...
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
                ...
                .with_label(
                    Label::new((path, range))
                        .with_message("add a `@pk(\"<field>\")` tag to this model")
                        .with_color(Color::Red),
                )
                .finish()
                .write(cache, std::io::stderr())
        ...
        SemanticError::ForeignKeyReferencesInvalidOrUnknownColumn { span, column } => {
            let (path, range) = span_parts(span, file_table);
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
                .with_message(format!("foreign key references unknown column '{column}'"))
                .with_label(
                    Label::new((path, range))
                        .with_message("this column does not exist on the referenced model")
                        .with_color(Color::Red),
                )
                .finish()
                .write(cache, std::io::stderr())
        ...
        SemanticError::ForeignKeyReferencesMultipleModels {
            span,
            first_model,
            second_model,
        } => {
            let (path, range) = span_parts(span, file_table);
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
                .with_message("foreign key references columns from multiple models")
                .with_label(
                    Label::new((path, range))
                        .with_message(format!(
                            "references both '{first_model}' and '{second_model}' — a foreign key must point to a single model"
                        ))
                        .with_color(Color::Red),
                )
                .finish()
        ...
        SemanticError::ForeignKeyReferencesIncompatibleColumnType {
            span,
            column,
            adj_column,
        } => {
            let (path, range) = span_parts(span, file_table);
            let (col_path, col_range) = span_parts(&column.span, file_table);
            let (adj_path, adj_range) = span_parts(&adj_column.span, file_table);
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
                .with_message("foreign key columns have incompatible types")
                .with_label(
                    Label::new((path, range))
                ...
                .with_label(
                    Label::new((col_path, col_range))
                        .with_message(format!(
                            "'{}' has type '{:?}'",
                            column.name, column.cidl_type
                        ))
                        .with_color(Color::Yellow),
                )
                .with_label(
                    Label::new((adj_path, adj_range))
                        .with_message(format!(
                            "'{}' has type '{:?}'",
                            adj_column.name, adj_column.cidl_type
                        ))
        ...
        SemanticError::ForeignKeyInconsistentNullability {
            span,
            first_column,
            second_column,
        } => {
            let (path, range) = span_parts(span, file_table);
            let (first_path, first_range) = span_parts(&first_column.span, file_table);
            let (second_path, second_range) = span_parts(&second_column.span, file_table);
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
                .with_message("foreign key columns have inconsistent nullability")
                .with_label(
                    Label::new((path, range))
                        .with_message("all columns in a foreign key must be consistently nullable or non-nullable")
                        .with_color(Color::Red),
                )
                .with_label(
                    Label::new((first_path, first_range))
                        .with_message(format!("'{}' declared here", first_column.name))
                        .with_color(Color::Yellow),
                )
                .with_label(
                    Label::new((second_path, second_range))
                        .with_message(format!("'{}' declared here", second_column.name))
                        .with_color(Color::Cyan),
                )
                .finish()
                .write(cache, std::io::stderr())
        ...
        SemanticError::ForeignKeyColumnAlreadyInForeignKey { span, column } => {
            let (path, range) = span_parts(span, file_table);
            let (col_path, col_range) = span_parts(&column.span, file_table);
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
                .with_message(format!(
                    "column '{}' is already part of another foreign key",
                    column.name
                ))
                .with_label(
                    Label::new((path, range))
                        .with_message("this foreign key references an already-claimed column")
                        .with_color(Color::Red),
                )
                .with_label(
                    Label::new((col_path, col_range))
                        .with_message(format!("'{}' first used here", column.name))
                        .with_color(Color::Yellow),
                )
                .finish()
                .write(cache, std::io::stderr())
        ...
        SemanticError::NavigationPropertyReferencesInvalidOrUnknownField { span, field } => {
            let (path, range) = span_parts(span, file_table);
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
                .with_message(format!(
                    "navigation property references unknown field '{field}'"
                ))
                .with_label(
                    Label::new((path, range))
                        .with_message("this field does not exist on the model")
                        .with_color(Color::Red),
                )
                .finish()
                .write(cache, std::io::stderr())
        ...
        SemanticError::NavigationPropertyReferencesDifferentDatabase { span, binding } => {
            let (path, range) = span_parts(span, file_table);
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
                .with_message(format!(
        ...
        SemanticError::NavigationPropertyFieldAlreadyInNavigationProperty { span, field } => {
            let (path, range) = span_parts(span, file_table);
            let (field_path, field_range) = span_parts(&field.span, file_table);
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
                .with_message(format!(
                    "field '{}' is already part of another navigation property",
                    field.name
                ))
                .with_label(
                    Label::new((path, range))
                        .with_message(
                            "this navigation property references an already-claimed field",
                        )
                        .with_color(Color::Red),
                )
                .with_label(
                    Label::new((field_path, field_range))
                        .with_message(format!("'{}' first used here", field.name))
                        .with_color(Color::Yellow),
                )
                .finish()
                .write(cache, std::io::stderr())
        ...
        SemanticError::NavigationPropertyMissingReciprocalM2M { span } => {
            let (path, range) = span_parts(span, file_table);
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
        ...
        SemanticError::NavigationPropertyAmbiguousM2M { span } => {
            let (path, range) = span_parts(span, file_table);
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
        ...
        SemanticError::UniqueConstraintReferencesInvalidOrUnknownField { span, field } => {
            let (path, range) = span_parts(span, file_table);
            Report::build(ariadne::ReportKind::Error, (path.clone(), range.clone()))
                .with_message(format!(
                    "unique constraint references unknown field '{field}'"
                ))
                .with_label(
                    Label::new((path, range))
                        .with_message("this field does not exist on the model")
