...
pub fn lex_and_parse(src: &str) -> ParseAst {
    let tokens = CloesceLexer.lex(src).expect("lex to succeed");
    CloesceParser.parse(tokens).expect("parse to succeed")
}

pub fn src_to_ast(src: &str) -> CloesceAst {
    let tokens = CloesceLexer.lex(src).expect("lex to succeed");
    let parse = CloesceParser.parse(tokens).expect("parse to succeed");
