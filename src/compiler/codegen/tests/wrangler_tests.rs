...
fn from_toml_str(src: &str) -> WranglerGenerator {
    WranglerGenerator::from_contents(src.to_string(), Path::new("wrangler.toml")).unwrap()
}

fn from_json_str(src: &str) -> WranglerGenerator {
    WranglerGenerator::from_contents(src.to_string(), Path::new("wrangler.json")).unwrap()
}
...
fn generates_default_wrangler_value() {
    ...
    let specs = vec![
        {
            let mut spec = from_toml_str("").as_spec(None).unwrap();
            WranglerDefault::set_defaults(&mut spec, &idl, "migrations");
            spec
        },
        {
            let mut spec = from_json_str("{}").as_spec(None).unwrap();
            WranglerDefault::set_defaults(&mut spec, &idl, "migrations");
            spec
        },
    ];
...
fn durable_objects_serialize_to_correct_wrangler_format() {
    ...
    // Act: TOML
    let mut toml_gen = from_toml_str("");
    ...
    // Act: JSON
    let mut json_gen = from_json_str("{}");
...
fn handles_d1_database_with_missing_values() {
    ...
    // Act
    let mut spec = from_toml_str(toml_with_incomplete_d1)
        .as_spec(None)
        .unwrap();
    ...
    let mut generator = from_toml_str(toml_with_incomplete_d1);
...
fn env_reads_from_env_block_and_falls_back_to_root() {
    ...
    let toml_spec = from_toml_str(toml_src).as_spec(Some("staging")).unwrap();
    assert_eq!(
    ...
    let toml_root_spec = from_toml_str(toml_src).as_spec(None).unwrap();
    assert_eq!(
    ...
    let json_spec = from_json_str(json_src).as_spec(Some("staging")).unwrap();
    assert_eq!(
    ...
    let json_root_spec = from_json_str(json_src).as_spec(None).unwrap();
    assert_eq!(
...
fn env_generate_writes_into_env_block() {
    ...
    // TOML: should write under env.staging
    {
        ...
        let mut generator = from_toml_str(toml_src);
        ...
        let root_spec = from_toml_str(&output).as_spec(None).unwrap();
        assert_eq!(
        ...
        let staging_spec = from_toml_str(&output).as_spec(Some("staging")).unwrap();
    ...
    // JSON: should write under env.staging
    {
        ...
        let mut generator = from_json_str(json_src);
        let mut spec = generator.as_spec(Some("staging")).unwrap();
        WranglerDefault::set_defaults(&mut spec, &idl, "migrations");
        let output = generator.generate(spec, Some("staging"));
        ...
        let staging_spec = from_json_str(&output).as_spec(Some("staging")).unwrap();
        assert!(staging_spec.d1_databases[0].database_id.is_some());
        assert!(staging_spec.kv_namespaces[0].id.is_some());
    }
}
...
fn durable_object_migrations_diff_against_existing_entries() {
    ...
    // Act
    let mut generator = from_toml_str(existing);
